...
<script type="text/javascript">
    	...
    	$(function () {
    		...
    		$('.option-select:eq('+(optionCount-1)+')').change(function () {
    				...
    				$td2_div_div3 = $('<div class="col-3 product-detail qtt-up-down"></div>');
    		...
    		$('#cart-btn').click(function () {
				...
				var cartDTO = new Object();
				cartDTO.cartList = cartList;
				console.log(cartDTO);
				$.ajax({
			...
			<div class="row">
				...
				<div class="col-md-12 col-lg-4">
					<nav aria-label="breadcrumb"
						...
						<span> ${product.completeCategory} </span>
					</nav>
				</div>
			</div>
		</div>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="agent-avatar-box" style="height: 85%;">
							...
							<img src="${cdnUrl}/${product.productImages[0].imagePath}" alt=""
					...
					<div class="col-md-5 section-md-t3">
						<div class="agent-info-box">
							...
							<div class="mb-3">
								...
								<div class="row mt-3 ml-3">
									<strong>Price: </strong> <span
										class="color-text-a span-price ml-5" id="price">${product.consumerPrice}원</span>
								</div>
								<div class="row mt-3 ml-3">
									<strong>Code: </strong> <span
										class="color-text-a span-code ml-5"> ${product.code}</span>
								</div>
								<c:forEach items="${product.options}" var="option"
									varStatus="index">
									<div class="row mt-3 ml-3">
										<strong>${option.optionKey}: </strong> <select
											class="form-control option-select">
											<option value="default">${option.optionKey}선택</option>
											<c:forEach items="${fn:split(option.optionValues,'/')}"
												var="optionValue">
							...
							<div>
								...
								<div id="totalProducts">
									<table border="1" summary="" class="table mt-2"
										style="text-align: center; border: none;">
										<tbody class="added-option" id="added-option"
											style="border: none;">
										</tbody>
										<tfoot>
											<tr>
												<td colspan="3">
													<div class="row">
														...
														<div class="col-4"></div>
														<input class="form-control text-center col-3 ml-2"
															value="0" id="totalPrice" readonly="readonly" type="text" />
														<span>원</span>
													</div>
												</td>
											</tr>
						...
						<div class="row cart-buy-btn">
							<div class="col-6">
								<button class="btn btn-warning btn-lg cart-btn" id="cart-btn"
									name="cart-btn">장바구니담기</button>
							</div>
							<div class="col-6">
				...
				<c:forEach items="${product.productImages}" var="productImage">
					<c:choose>
						<c:when test="${productImage.mainImageCheck!='Y'}">
							<div class="container">
								<div class="img-box-a">
									<img src="${cdnUrl}/${productImage.imagePath}" alt=""
										class="img-a img-fluid w-100">
								</div>
							</div>
						</c:when>
					</c:choose>
